...
export const useBorrowPositions = () => {
	...
	Object.keys(sortedByCollateral).forEach((coll) => {
		...
		items.forEach((i) => {
			...
			if (getEffectiveExpiration(i) > getEffectiveExpiration(bestExpiration)) bestExpiration = i;

			if (BigInt(i.availableForClones) > BigInt(bestAvailability.availableForClones)) bestAvailability = i;
