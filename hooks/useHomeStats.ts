...
export const useHomeStats = () => {
  ...
  const frankenTotalSupply: bigint = data
    ? decodeBigIntCall(data[0])
    : BigInt(0);
  ...
  const frankenMinterReserve: bigint = data
    ? decodeBigIntCall(data[4])
    : BigInt(0);
  ...
  const equityTotalSupply: bigint = data
    ? decodeBigIntCall(data[9])
    : BigInt(0);
  const equityMarketCap: bigint =
    (equityPrice * equityTotalSupply) / BigInt(1e18);
  const equityBalance: bigint = data ? decodeBigIntCall(data[10]) : BigInt(0);
  const equityTotalVotes: bigint = data
    ? decodeBigIntCall(data[11])
    : BigInt(0);
