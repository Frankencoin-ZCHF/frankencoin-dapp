...
const getMiscelaneousLoanDetails = (position: PositionQuery, loanAmount: bigint, collateralAmount: bigint) => {
	...
	const liquidationPriceAtEnd = collateralAmount === 0n ? BigInt(0) : (loanAmount + interestUntilExpiration) * (BigInt(10) ** BigInt(collateralDecimals)) / collateralAmount;
...
export const getLoanDetailsByCollateralAndStartingLiqPrice = (position: PositionQuery, collateralAmount: bigint, startingLiquidationPrice: bigint): LoanDetails => {
	...
	const liquidationPriceAtEndOfPeriod = collateralAmount === 0n ? BigInt(0) : ((loanAmountStartOfPeriod + interestUntilExpiration) * (BigInt(10) ** BigInt(collateralDecimals))) / collateralAmount;
